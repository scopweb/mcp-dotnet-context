...
impl GenericAnalyzer {
    ...
    fn parse_dotnet_project(path: &Path) -> Result<(String, Option<String>, Vec<Dependency>, ProjectMetadata)> {
        // Find .csproj file
        let csproj = Self::find_file_with_extension(path, "csproj")?;
        let content = fs::read_to_string(&csproj)?;
        
        let name = csproj.file_stem()
            .and_then(|s| s.to_str())
            .unwrap_or("Unknown")
            .to_string();
    ...
    fn parse_rust_project(path: &Path) -> Result<(String, Option<String>, Vec<Dependency>, ProjectMetadata)> {
    ...
    fn parse_node_project(path: &Path) -> Result<(String, Option<String>, Vec<Dependency>, ProjectMetadata)> {
        ...
        if json["dependencies"].get("react").is_some() {
            metadata.extra.insert("framework".to_string(), "react".to_string());
        } else if json["dependencies"].get("vue").is_some() {
            metadata.extra.insert("framework".to_string(), "vue".to_string());
        } else if json["dependencies"].get("next").is_some() {
            metadata.extra.insert("framework".to_string(), "next".to_string());
        }
        
        Ok((name, version, dependencies, metadata))
    }
    
    fn parse_python_project(path: &Path) -> Result<(String, Option<String>, Vec<Dependency>, ProjectMetadata)> {
    ...
    fn parse_go_project(path: &Path) -> Result<(String, Option<String>, Vec<Dependency>, ProjectMetadata)> {
    ...
    fn parse_java_project(path: &Path) -> Result<(String, Option<String>, Vec<Dependency>, ProjectMetadata)> {
        let mut name = path.file_name()
            .and_then(|s| s.to_str())
            .unwrap_or("Unknown")
            .to_string();
    ...
    fn parse_php_project(path: &Path) -> Result<(String, Option<String>, Vec<Dependency>, ProjectMetadata)> {
        ...
        if let Some(deps) = json["require"].as_object() {
            if let Some(php_ver) = deps.get("php") {
                metadata.extra.insert("php_version".to_string(), 
            ...
            if let Ok(pkg_content) = fs::read_to_string(&package_json) {
                if let Ok(pkg_json) = serde_json::from_str::<serde_json::Value>(&pkg_content) {
                    ...
                    if pkg_json["dependencies"].get("vue").is_some() 
                        ...
                        metadata.extra.insert("frontend".to_string(), "vue".to_string());
                    }
                    ...
                    if pkg_json["dependencies"].get("react").is_some() 
                        ...
                        metadata.extra.insert("frontend".to_string(), "react".to_string());
                    }
                    // Check for Vite
                    if pkg_json["devDependencies"].get("vite").is_some() {
                        metadata.extra.insert("bundler".to_string(), "vite".to_string());
                    }
                    // Check for Laravel Mix / Webpack
                    if pkg_json["devDependencies"].get("laravel-mix").is_some() {
                        metadata.extra.insert("bundler".to_string(), "laravel-mix".to_string());
                    }
                }
            }
        }
        ...
        if metadata.extra.get("framework").map(|f| f == "laravel").unwrap_or(false) {
            metadata.entry_point = Some("public/index.php".to_string());
            metadata.build_command = Some("php artisan serve".to_string());
        } else if metadata.extra.get("framework").map(|f| f == "symfony").unwrap_or(false) {
            metadata.entry_point = Some("public/index.php".to_string());
    ...
    fn detect_php_framework(dependencies: &[Dependency], path: &Path) -> Option<String> {
        ...
        if dependencies.iter().any(|d| d.name.starts_with("symfony/")) {
            ...
            if dependencies.iter().any(|d| d.name == "symfony/framework-bundle") {
        ...
        if dependencies.iter().any(|d| d.name == "codeigniter4/framework") {
    ...
    fn parse_unknown_project(path: &Path) -> Result<(String, Option<String>, Vec<Dependency>, ProjectMetadata)> {
